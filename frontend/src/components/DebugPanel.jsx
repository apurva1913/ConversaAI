...
export default function DebugPanel({ log, sessionId, sessionStatus, messages }) {
  const [collapsed, setCollapsed] = useState(false);
  const [activeTab, setActiveTab] = useState('insights'); // 'insights' | 'debug'

  const lastAiMsg = [...(messages || [])].reverse().find(m => m.role === 'assistant');
  const rpaData = lastAiMsg?.rpaData;
  const sources = lastAiMsg?.sources;
  ...
  return (
    <div className="debug-sidebar">
      <div className="debug-header" style={{ display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'stretch' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div className="debug-title">
            <span>⚙️</span> Session Context
          </div>
          <button
        ...
        <div style={{ display: 'flex', gap: 4, background: 'var(--bg-tertiary)', padding: 4, borderRadius: 'var(--radius)' }}>
          <button 
            style={{
              flex: 1, padding: '6px 0', fontSize: 12, fontWeight: 600, borderRadius: 6,
              background: activeTab === 'insights' ? 'var(--bg-card)' : 'transparent',
              color: activeTab === 'insights' ? 'var(--text-primary)' : 'var(--text-muted)',
          ...
          <button 
            style={{
              flex: 1, padding: '6px 0', fontSize: 12, fontWeight: 600, borderRadius: 6,
              background: activeTab === 'debug' ? 'var(--bg-card)' : 'transparent',
              color: activeTab === 'debug' ? 'var(--text-primary)' : 'var(--text-muted)',
              boxShadow: activeTab === 'debug' ? 'var(--shadow-sm)' : 'none',
              transition: 'all 0.2s'
            }}
        ...
        {activeTab === 'insights' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className="debug-section">
              <div className="debug-section-header">Session Info</div>
              <div className="debug-section-body">
                <div className="debug-kv">
                  <span className="debug-key">Session ID</span>
                  <span className="debug-val">{sessionId?.slice(0, 8)}…</span>
                </div>
                <div className="debug-kv">
                  <span className="debug-key">Status</span>
                  <span className="debug-val" style={{ color: sessionStatus === 'live_agent' ? 'var(--green)' : 'var(--blue)' }}>
                    {sessionStatus === 'live_agent' ? 'Escalated (Live Agent)' : 'Active (AI)'}
                  </span>
                </div>
                <div className="debug-kv">
                  <span className="debug-key">Messages</span>
                  <span className="debug-val" style={{ color: 'var(--text-secondary)' }}>
                    {messages?.length || 0}
                  </span>
                </div>
              </div>
            ...
            {rpaData && (
              <div className="debug-section" style={{ borderColor: 'rgba(16, 185, 129, 0.3)' }}>
                <div className="debug-section-header" style={{ color: 'var(--green)' }}>🤖 Extracted Data (RPA)</div>
                <div className="debug-section-body">
                  {Object.entries(rpaData).map(([k, v]) => (
                    <div key={k} className="debug-kv" style={{ marginBottom: 8 }}>
                      <span className="debug-key" style={{ textTransform: 'capitalize', width: 'auto', minWidth: 80 }}>{k}</span>
                      <span className="debug-val" style={{ color: 'var(--text-primary)' }}>
            ...
            {sources && sources.length > 0 && (
              <div className="debug-section" style={{ borderColor: 'rgba(59, 130, 246, 0.3)' }}>
                <div className="debug-section-header" style={{ color: 'var(--blue)' }}>📚 Knowledge Context</div>
                <div className="debug-section-body" style={{ padding: 0 }}>
                  {sources.map((ctx, i) => (
                    <div key={i} style={{ padding: '8px 12px', borderBottom: i < sources.length - 1 ? '1px solid var(--border)' : 'none' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)' }}>{ctx.source}</span>
                        {ctx.score != null && (
            ...
            {!rpaData && (!sources || sources.length === 0) && (
              <div className="debug-empty" style={{ padding: '20px' }}>
                <span style={{ fontSize: 24, opacity: 0.5 }}>📊</span>
                <span>Send a query to see extracted context, knowledge sources, or RPA parameters here.</span>
              </div>
            )}
          </div>
        )}
